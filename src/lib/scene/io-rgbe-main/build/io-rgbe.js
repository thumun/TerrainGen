var RGBE = (function (exports) {
  ...
  function encodeRGBE(imageData) {
    ...
    for (var y = 0; y < height; ++y) {
      ...
      scanline.forEach(function (s) {
        return encodeRLE(s, encoded);
      });
    }
  ...
  function encodeRLE(data, encoded) {
    ...
    while (i < len) {
      ...
      while (i < runStart) {
        ...
        encoded.push(count);
        for (var d = 0; d < count; ++d) encoded.push(data[i + d]);
        i += count;
      }
